...
def index():
    ...
    template_options = [
        f"{index + 1}、{template}" for index, template in enumerate(template_list)
    ]
    ...
    return render_template(
        ...
        providers_data=json.dumps(providers_data, indent=4, ensure_ascii=False),
...
def config(url):
    ...
    if rua_values and any(
        rua_value in user_agent for rua_value in rua_values.split(",")
    ):
        return Response(
            json.dumps(
                {"status": "error", "message": "block"}, indent=4, ensure_ascii=False
            ),
            content_type="application/json; charset=utf-8",
            status=403,
        )
    ...
    if substrings and any(substring in url for substring in substrings.split(",")):
        return Response(
            json.dumps(
                {"status": "error", "message_CN": "填写参数不符合规范"},
                indent=4,
                ensure_ascii=False,
            ),
            content_type="application/json; charset=utf-8",
    ...
    if not query_string:
        if any(
            substring in encoded_url
            for substring in ["&emoji=", "&file=", "&eps=", "&enn="]
        ):
            if "|" in encoded_url:
            ...
            if request.args.get("file"):
                ...
                if index != -1:
                    if (
                        next_index < len(request.args["file"])
                        and request.args["file"][next_index] != "/"
                    ):
                        request.args["file"] = (
                            request.args["file"][: next_index - 1]
                            + "/"
                            + request.args["file"][next_index - 1 :]
                        )
    else:
        if any(
            substring in query_string
            for substring in ["&emoji=", "&file=", "&eps=", "&enn="]
        ):
    ...
    if len(url_parts) > 1:
        subscribe["url"] = (
            full_url.split("url=", 1)[-1].split("|")[0]
            if full_url.startswith("url")
            else full_url.split("|")[0]
        )
        subscribe["ex-node-name"] = enn_param
        subscribe2["url"] = (
            full_url.split("url=", 1)[-1].split("|")[1]
            if full_url.startswith("url")
            else full_url.split("|")[1]
        )
        subscribe2["emoji"] = 1
        ...
        if len(url_parts) == 3:
            subscribe3["url"] = (
                full_url.split("url=", 1)[-1].split("|")[2]
                if full_url.startswith("url")
                else full_url.split("|")[2]
            )
            subscribe3["enabled"] = True
            subscribe3["ex-node-name"] = enn_param
    if len(url_parts) == 1:
        subscribe["url"] = (
            full_url.split("url=", 1)[-1] if full_url.startswith("url") else full_url
        )
        subscribe["emoji"] = (
            int(emoji_param) if emoji_param.isdigit() else subscribe.get("emoji", "")
        )
    ...
    temp_json_data["exclude_protocol"] = (
        eps_param if eps_param else temp_json_data.get("exclude_protocol", "")
    )
    temp_json_data["config_template"] = (
        unquote(file_param) if file_param else temp_json_data.get("config_template", "")
    )
